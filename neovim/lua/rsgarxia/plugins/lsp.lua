    ...
    "folke/neodev.nvim",
    {
      ...
      config = function()
          ...
          local function on_attach(_, bufnr)
              ...
              buf_set_keymap("n", "gr", '', { desc = "(LSP) List all the references to the symbol", callback = function () vim.lsp.buf.references(nil, { on_list = on_list }) end })
              buf_set_keymap("n", "gD", "<cmd>lua vim.lsp.buf.declaration()<CR>",  { desc = "(LSP) Go to declaration" })
              buf_set_keymap("n", "gd", "<cmd>lua vim.lsp.buf.definition()<CR>", { desc = "(LSP) Go to definition" })
              buf_set_keymap("n", "gi", "<cmd>lua vim.lsp.buf.implementation()<CR>", { desc = "(LSP) Go to implementation" })
              buf_set_keymap("n", "K", "<cmd>lua vim.lsp.buf.hover()<CR>", { desc = "(LSP) Hover" })
              buf_set_keymap("n", "<C-k>", "<cmd>lua vim.lsp.buf.signature_help()<CR>", { desc = "(LSP) Show signature help" })
              buf_set_keymap("n", "<leader>D", "<cmd>lua vim.lsp.buf.type_definition()<CR>", { desc = "(LSP) Jump to the definition of the type" })
              buf_set_keymap("n", "<leader>rn", "<cmd>lua vim.lsp.buf.rename()<CR>", { desc = "(LSP) Rename variable" })
              buf_set_keymap("n", "<leader>ca", "<cmd>lua vim.lsp.buf.code_action()<CR>", { desc = "(LSP) Select a code action available for the current position" })
              buf_set_keymap("n", "<leader>q", "<cmd>lua vim.diagnostic.setloclist()<CR>", { desc = "(LSP) Add buffer diagnostic to the location list" })
          end
